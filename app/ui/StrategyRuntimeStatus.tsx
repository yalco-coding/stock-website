...
type RuntimeStatus = { mode: "mock"; environments: EnvironmentHealth[]; positions: RuntimePosition[]; activeOrders: { orderNo: string | null; state: string; code?: string; needsAttention: boolean }[] };
...
export function StrategyRuntimeStatus() {
  ...
  return <div className="mt-6 border-t border-slate-100 pt-5">
    ...
    {query.isError ? <p className="mt-3 text-xs text-rose-600">{query.error.message}</p> : query.data && <>
      <p className="mt-2 text-xs text-slate-500">모의투자 자동 실행 · 각 전략의 ON/OFF 설정에 따라 동작합니다.</p>
