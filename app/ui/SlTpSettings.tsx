...
import { DEFAULT_STRATEGY_SETTINGS, MARKET_SCHEDULES, type ExcludedStock, type SlTpMarketSettings, type StrategyMarket, type StrategySettings } from "../strategy-settings";
...
export function SlTpSettings({ environment }: { environment: Environment }) {
  ...
  const current = settings.strategies.slTp[market]; const schedule = MARKET_SCHEDULES[market];
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      setSaving(true);
      try {
        const response = await fetch("/api/settings/strategies", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ strategy: "slTp", market, patch: settings.strategies.slTp[market], expectedRevision: serverRevision.current }) });
  ...
  const update = (next: Partial<SlTpMarketSettings>) => { if (real) return; changeRevision.current += 1; setSaveNotice(""); setSettings((value) => ({ ...value, strategies: { ...value.strategies, slTp: { ...value.strategies.slTp, [market]: { ...value.strategies.slTp[market], ...next } } } })); };
