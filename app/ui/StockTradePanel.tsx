...
export function StockTradePanel({ stock, environment, mode = "buy", holding, onClose }: { stock: TradableStock | null; environment: Environment; mode?: "buy" | "sell"; holding?: { quantity: number; availableQuantity: number }; onClose: () => void }) {
  ...
  const supported = !!stock && (mode === "sell" ? (holding?.availableQuantity ?? 0) > 0 : environment === "mock-domestic" ? stock.marketCode === "KRX" : ["NA", "ND", "NY"].includes(stock.marketCode));
  const unsupportedReason = environment === "mock-domestic" ? "국내 모의투자는 KRX 종목만 매수할 수 있습니다." : "해외 모의투자는 NASDAQ·NYSE·AMEX 종목만 매수할 수 있습니다.";
  ...
  const currency = detail.data?.currency ?? (environment === "mock-domestic" ? "KRW" : "USD");
  ...
  return <><button className="fixed inset-0 z-40 bg-slate-950/30 backdrop-blur-[1px]" onClick={onClose} aria-label="거래 패널 닫기"/><aside className="fixed inset-y-0 right-0 z-50 flex w-full max-w-md flex-col overflow-y-auto border-l border-slate-200 bg-[#f7f9f7] shadow-2xl" role="dialog" aria-modal="true" aria-label={`${stock.name} 거래 패널`}>
    ...
    <div className="space-y-5 p-5">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5"><div className="mb-4 flex items-center gap-2"><ShoppingCart size={18} className="text-emerald-800"/><h3 className="font-bold text-slate-900">모의투자 {mode === "sell" ? "매도" : "매수"}</h3></div>
