...
import { normalizeDomesticOrderPrice } from "../domestic-order-price";
...
type OrderStatus = { trId: string; state: string; status: "pending" | "partial" | "filled"; label: string; source: "manual" | "strategy" | "external"; reasons: string[]; needsAttention: boolean; orderedQuantity?: number; filledQuantity: number; remainingQuantity: number; filledPrice?: number };
...
export function StockTradePanel({ stock, environment, mode = "buy", holding, onClose }: { stock: TradableStock | null; environment: Environment; mode?: "buy" | "sell"; holding?: { quantity: number; availableQuantity: number }; onClose: () => void }) {
  ...
  const enteredPrice = Number(price || detail.data?.currentPrice || 0);
  const effectivePrice = domestic && orderType === "limit"
    ? normalizeDomesticOrderPrice(enteredPrice, stock?.market ?? "", stock?.isEtf, mode)
    : enteredPrice;
  ...
  const status = useQuery({
    ...
    enabled: !!order && !!stock,
    refetchInterval: (query) => ["filled", "rejected", "cancelled-partial", "manual-review"].includes(query.state.data?.state ?? "") ? false : 5_000,
  });
