...
import { CoordinatedOrderRejectedError, CoordinatedOrderUnknownError, publicOrder, submitCoordinatedOrder } from "../../../strategy-engine/order-coordinator.server";
...
export async function POST(request: NextRequest) {
  ...
  if (domestic ? body.marketCode !== "KRX" : !["NA", "ND", "NY"].includes(body.marketCode ?? "")) {
    return NextResponse.json({ message: domestic ? "국내 모의투자는 KRX 종목만 지원합니다." : "해외 모의투자는 미국 거래소 종목만 지원합니다." }, { status: 400 });
  }
  ...
  try {
    ...
    if (error instanceof CoordinatedOrderRejectedError) return NextResponse.json({ message: error.message }, { status: 422, headers: { "Cache-Control": "no-store" } });
    if (error instanceof CoordinatedOrderUnknownError) return NextResponse.json({ message: `${error.message} 주문내역 확인이 필요합니다.`, activeOrder: publicOrder(error.order) }, { status: 502, headers: { "Cache-Control": "no-store" } });
