...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (!domestic && !["NA", "ND", "NY"].includes(marketCode)) marketCode = await resolveUsMarket(code, token);
    const trId = domestic ? "ka10001" : "usa20100";
    const response = await fetch(`${MOCK_DOMAIN}${domestic ? "/api/dostk/stkinfo" : "/api/us/mrkcond"}`, { method: "POST", headers: { "Content-Type": "application/json;charset=UTF-8", "api-id": trId, authorization: `Bearer ${token}` }, body: JSON.stringify(domestic ? { stk_cd: code } : { stex_tp: marketCode, stk_cd: code }), cache: "no-store" });
    ...
    return NextResponse.json({ trId, marketCode: domestic ? "KRX" : marketCode, code: String(body.stk_cd ?? code), name: String(body.stk_nm ?? ""), englishName: domestic ? undefined : String(body.stk_enm ?? ""), currency: domestic ? "KRW" : "USD", currentPrice: num(body.cur_prc), change: signedNum(body.pred_pre), changeRate: signedNum(body.flu_rt), openPrice: num(body.open_pric), highPrice: num(body.high_pric), lowPrice: num(body.low_pric), volume: num(domestic ? body.trde_qty : body.acc_trde_qty), suspended: domestic ? false : body.trd_susp_tp !== "0" }, { headers: { "Cache-Control": "no-store" } });
  } catch (error) {
