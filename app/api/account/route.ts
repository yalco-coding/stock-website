...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const positions = domestic
      ? (body.acnt_evlt_remn_indv_tot ?? []).map((p) => ({ code: String(p.stk_cd ?? "").replace(/^[AJQ]/, ""), name: String(p.stk_nm ?? ""), quantity: num(p.rmnd_qty), availableQuantity: num(p.trde_able_qty), averagePrice: num(p.pur_pric), currentPrice: Math.abs(num(p.cur_prc)), purchaseAmount: num(p.pur_amt), evaluationAmount: num(p.evlt_amt), profitLoss: num(p.evltv_prft), returnRate: num(p.prft_rt) }))
      : (body.result_list ?? []).map((p) => ({ code: String(p.stk_cd ?? ""), name: String(p.frgn_stk_nm ?? ""), market: String(p.stex_nm ?? ""), quantity: num(p.poss_qty), availableQuantity: num(p.sell_alowq), averagePrice: num(p.frgn_stk_book_uv), currentPrice: num(p.now_pric), purchaseAmount: num(p.frgn_stk_book_amt), evaluationAmount: num(p.evlt_amt), profitLoss: num(p.pl_amt), returnRate: num(p.pl_rt) }));
    let cash;
    if (!domestic) {
