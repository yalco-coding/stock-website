...
export async function GET(request: NextRequest) {
  ...
  if (!isInvestmentEnvironment(requested)) return NextResponse.json({ message: "지원하지 않는 투자 환경입니다." }, { status: 400 });
  ...
  try {
    ...
    const response = await fetch(`${domain}${domestic ? "/api/dostk/acnt" : "/api/us/acnt"}`, { method: "POST", headers: { ...apiHeaders, "api-id": trId }, body: JSON.stringify(domestic ? { qry_tp: "1", dmst_stex_tp: "KRX" } : { stex_tp: "", stk_cd: "" }), cache: "no-store" });
